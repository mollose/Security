...
#### <ins>Location</ins>
API 후킹을 위해 어느 부분을 공략해야 하는지에 대한 내용
* IAT : IAT에 있는 API 주소를 후킹 함수 주소로 변경. IAT에 없지만 프로그램에서 사용되는 API에 대해서는 후킹 불가(ex. DLL의 동적 로딩)
* Code : 프로세스 메모리에 매핑된 시스템 라이브러리에서 API의 실제 주소를 찾아가 코드를 직접 수정하는 방법. 시작 코드를 JMP 명령어로 패치하는 방법, 함수 일부를 덮어쓰는 방법, 필요한 부분만 일부 변경하는 방법 등
* EAT : DLL의 EAT에 기록된 API 시작 주소를 후킹 함수 주소로 변경

#### <ins>Technique(How)</ins>
후킹 대상 프로세스 메모리에 침투하여 후킹 함수를 설치하는 구체적 기법
* Debug : 디버거는 디버기에 대한 모든 권한(실행 제어, 메모리 액세스 등)을 갖기 때문에, 디버기의 프로세스 메모리에 후킹 함수를 자유롭게 설치 가능. 이때의 디버거는 일반적인 OllyDbg 등의 포르램이 아니라, 후킹을 위해 사용자가 직접 제작한 프로그램. 프로그램에서 Debug API를 이용하여 대상 프로세스에 Attach하고, 실행이 잠시 멈춘 상태에서 후킹 함수를 설치. 물론 기존 디버거에서 자동화 스크립트를 이용하여 API 후킹을 자동화하는 방법도 존재(API 후킹 도중 Interactive하게 후킹을 추가, 수정, 제거할 수 있으나, 디버거에 대한 지식과 많은 테스트가 요구됨)

<br/><br/>

## 예제 #8: 메모장 WriteFile() 후킹

<br/>

### 디버그 테크닉
사용자와 좀 더 interactive한 후킹 수행 가능. 사용자에게 간단한 인터페이스를 제공하여 후킹 대상 프로그램의 실행을 제어하고, 메모리를 자유롭게 사용 가능. 디버거는 디버기의 명령어를 하나씩 실행할 수 있으며, 레지스터와 메모리에 대한 모든 접근권한을 가짐
* 일단 디버거 프로세스로 등록되면, OS는 디버기에서 디버그 이벤트가 발생할 때마다 디버기의 실행을 멈추고 해당 이벤트를 디버거에게 통보. 디버거는 해당 이벤트에 대해 적절한 처리를 한 후 디버기의 실행을 재개할 수 있음
  * 일반적인 예외(Exception)도 디버그 이벤트에 해당
  * 만약 해당 프로세스가 디버깅 중이 아니라면, 디버그 이벤트는 자체 예외처리 혹은 OS의 예외처리 루틴에서 처리됨
  * 디버거는 디버그 이벤트 중에서 처리할 수 없거나 관심 없는 이벤트들은 OS가 처리하도록 만듦
* 디버그 이벤트 중 디버깅과 관련된 이벤트는 EXCEPTION_DEBUG_EVENT이며, 관련된 예외 목록 중 디버거가 반드시 처리해야하는 예외는 EXCEPTION_BREAKPOINT(BP의 어셈블리 명령어는 INT 3이며, IA-32 Intruction으로는 0xCC. 코드 실행 중 INT 3 명령어를 만나면 실행이 중지되고 디버거에게 EXCEPTION_BREAKPOINT 예외 이벤트가 전달됨)

<br/>

### 작업 순서
1) 후킹을 원하는 프로세스에 ‘attach’하여, 해당 프로세스를 디버기로 만듦
2) 훅 : API 시작 주소의 첫 바이트를 0xCC로 변경
3) API가 호출되면 제어는 디버거에게 넘어옴
4) 원하는 작업을 수행(파라미터와 리턴값 조작 등)
5) 언훅 : API의 정상 실행을 위해 0xCC를 원래 값으로 복원시킴
